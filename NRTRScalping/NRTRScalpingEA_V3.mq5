...
//|  1. NRTR read from external NRTR Channel indicator (iCustom)    |
//|     using trend/level/extreme buffers                            |
...
input string NRTRPath              = "NRTR Channel"; // iCustom path/name
input int    NRTR_ATRPeriod        = 14;             // Indicator ATR period
input double NRTR_K                = 2.0;            // Indicator ATR multiplier
input int    NRTRTrendBufferIndex  = 4;              // Trend buffer (1/-1)
input int    NRTRUpLevelBuffer     = 1;              // Uptrend ratchet/support buffer
input int    NRTRDownLevelBuffer   = 2;              // Downtrend ratchet/support buffer
input int    NRTRUpExtremeBuffer   = 0;              // Uptrend extreme/resistance buffer
input int    NRTRDownExtremeBuffer = 3;              // Downtrend extreme/resistance buffer
...
CTrade trade;
int g_nrtrHandleMain  = INVALID_HANDLE;
int g_nrtrHandleLower = INVALID_HANDLE;

//+------------------------------------------------------------------+
//| Read one value from indicator buffer                             |
//+------------------------------------------------------------------+
bool ReadIndicatorValue(int handle, int bufferIndex, int shift, double &value)
{
   if(handle == INVALID_HANDLE || bufferIndex < 0 || shift < 0)
      return false;
...
//+------------------------------------------------------------------+
//| Read NRTR state from external indicator on chart TF              |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Read NRTR state from external indicator on specific TF          |
//+------------------------------------------------------------------+
NRTRState CalcNRTRAtBar_TF(ENUM_TIMEFRAMES tf, int barIdx)
{
   ...
   if(tf == _Period)
      handle = g_nrtrHandleMain;
   else if(tf == LowerTF)
      handle = g_nrtrHandleLower;
   else
      return s;

   double trendValue = 0.0;
   if(!ReadIndicatorValue(handle, NRTRTrendBufferIndex, barIdx, trendValue))
      return s;

   if(trendValue > 0.0)
      s.direction = 1;
   else if(trendValue < 0.0)
      s.direction = -1;
   else
      return s;
   ...
   if(s.direction == 1)
   {
      ReadIndicatorValue(handle, NRTRUpLevelBuffer, barIdx, s.level);
      ReadIndicatorValue(handle, NRTRUpExtremeBuffer, barIdx, s.extreme);
   }
   else
   {
      ReadIndicatorValue(handle, NRTRDownLevelBuffer, barIdx, s.level);
      ReadIndicatorValue(handle, NRTRDownExtremeBuffer, barIdx, s.extreme);
   }

   if(s.level == EMPTY_VALUE)   s.level = 0.0;
   if(s.extreme == EMPTY_VALUE) s.extreme = 0.0;
...
int OnInit()
{
   ...
   g_nrtrHandleMain = iCustom(_Symbol, _Period, NRTRPath, NRTR_ATRPeriod, NRTR_K, false);
   if(g_nrtrHandleMain == INVALID_HANDLE)
   {
      Print("NRTR main handle failed: ", GetLastError(), " Path=", NRTRPath);
      return INIT_FAILED;
   }

   g_nrtrHandleLower = iCustom(_Symbol, LowerTF, NRTRPath, NRTR_ATRPeriod, NRTR_K, false);
   if(g_nrtrHandleLower == INVALID_HANDLE)
   {
      Print("NRTR lower handle failed: ", GetLastError(), " TF=", EnumToString(LowerTF));
      IndicatorRelease(g_nrtrHandleMain);
      g_nrtrHandleMain = INVALID_HANDLE;
      return INIT_FAILED;
   }
   ...
   RebuildFromOpenPositions();
      ...
      Print("NRTRScalpingEA V3 initialized.  Symbol: ", _Symbol,
...
void OnDeinit(const int reason)
{
   if(g_nrtrHandleMain != INVALID_HANDLE)
      IndicatorRelease(g_nrtrHandleMain);
   if(g_nrtrHandleLower != INVALID_HANDLE)
      IndicatorRelease(g_nrtrHandleLower);

   g_nrtrHandleMain  = INVALID_HANDLE;
   g_nrtrHandleLower = INVALID_HANDLE;
