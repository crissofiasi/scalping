...
int OnInit()
{
   ...
   handleADX = iADX(_Symbol, InpADX_Timeframe, InpADX_Period);
   ...
   Print("ADX Trend Mode: ", InpUseADXTrendMode ? "Enabled" : "Disabled",
         " | ADX Period: ", InpADX_Period,
         " | Threshold: ", DoubleToString(InpADX_Threshold, 1));
...
bool UpdateIndicators()
{
   ...
   if(CopyBuffer(handleBB, 2, 0, 1, bb_lower_array) < 1)  // Lower band
      return false;
   if(CopyBuffer(handleADX, 0, 0, 1, adx_array) < 1)
      return false;
...
void CheckForEntry()
{
   ...
   if(InpDebugMode && TimeCurrent() - lastDebugTime > 60) // Print every minute
   {
      ...
      Print("ADX: ", DoubleToString(adx_current, 2));
      Print("Band Width: ", DoubleToString(PriceToPips(bb_upper - bb_lower), 2), " pips");
      lastDebugTime = TimeCurrent();
   }

   bool strongTrend = InpUseADXTrendMode && (adx_current >= InpADX_Threshold);
   double minBandDistance = PipsToPrice(InpMinBandDistance);

   if(strongTrend)
   {
      double trendBuyPrice = (InpTrendScalpSignal == TREND_SCALP_CLOSE) ? rates[0].close : rates[0].high;
      double trendSellPrice = (InpTrendScalpSignal == TREND_SCALP_CLOSE) ? rates[0].close : rates[0].low;
      bool trendBuySignal = trendBuyPrice > bb_upper;
      bool trendSellSignal = trendSellPrice < bb_lower;
      bool trendBuyCloseOk = !InpRequireCandleClose || (rates[0].close > bb_upper);
      bool trendSellCloseOk = !InpRequireCandleClose || (rates[0].close < bb_lower);
      bool trendBuyDistanceOk = (InpMinBandDistance <= 0.0) || ((trendBuyPrice - bb_upper) >= minBandDistance);
      bool trendSellDistanceOk = (InpMinBandDistance <= 0.0) || ((bb_lower - trendSellPrice) >= minBandDistance);

      if(trendBuySignal && trendBuyCloseOk && trendBuyDistanceOk)
      {
         if(InpDebugMode)
         {
      ...
      if(trendSellSignal && trendSellCloseOk && trendSellDistanceOk)
      {
         if(InpDebugMode)
         {
   ...
   bool closedAboveLowerBand = !InpRequireCandleClose || (rates[0].close > bb_lower);
   bool buyDistanceOk = (InpMinBandDistance <= 0.0) || ((rates[0].close - bb_lower) >= minBandDistance);
   
   if(touchedLowerBand && closedAboveLowerBand && buyDistanceOk)
   {
      if(InpDebugMode)
      {
   ...
   bool closedBelowUpperBand = !InpRequireCandleClose || (rates[0].close < bb_upper);
   bool sellDistanceOk = (InpMinBandDistance <= 0.0) || ((bb_upper - rates[0].close) >= minBandDistance);
   
   if(touchedUpperBand && closedBelowUpperBand && sellDistanceOk)
   {
      if(InpDebugMode)
      {
...
void OpenTrade(ENUM_ORDER_TYPE orderType, bool isTrendScalp)
{
   ...
   double sl = CalculateStopLoss(orderType, price, isTrendScalp);
   double tp = CalculateTakeProfit(orderType, price, isTrendScalp);
...
//| Calculate stop loss                                              |
//+------------------------------------------------------------------+
double CalculateStopLoss(ENUM_ORDER_TYPE orderType, double entryPrice, bool isTrendScalp)
{
   if(isTrendScalp)
   {
      return bb_middle;
...
//+------------------------------------------------------------------+
double CalculateTakeProfit(ENUM_ORDER_TYPE orderType, double entryPrice, bool isTrendScalp)
{
   if(isTrendScalp)
   {
      double slDistance = MathAbs(entryPrice - bb_middle);
      if(orderType == ORDER_TYPE_BUY)
